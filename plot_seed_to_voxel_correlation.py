...
func_filename = sys.argv[2]
output_dir = sys.argv[3]


##########################################################################
# Note that func_filename is strings pointing to files on your hard drive.
print(seed_name)
print(func_filename)
...
coords = [tuple([int(i) for i in sys.argv[4:7]])]
print(coords)
...
# sphere is centered at coords and will have the radius we pass the
...
##########################################################################
# Then we extract the mean time series within the seed region
...
##########################################################################
# Then we extract the brain-wide voxel-wise time series 
...
output_nii_path = os.path.join(output_dir, '%s.nii.gz' % seed_name)
seed_based_correlation_img.to_filename(output_nii_path)
...
output_pdf_path = os.path.join(output_dir, '%s.pdf' % seed_name)
display.savefig(output_pdf_path)
